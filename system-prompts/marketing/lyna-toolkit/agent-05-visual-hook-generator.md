...
## Step 2 — No-Text Overlay Rule

We do not want any text overlays on our images. Every hook prompt must produce a completely clean, textless image. 

To guarantee that image generation models do not attempt to render text inside the image:
1. Every generated image prompt must explicitly specify: `No text.`, `No text overlay.`, `Clean image with no typography or text.`.
...
iPhone-camera aesthetic. Natural smartphone perspective. Mostly everything in focus. UGC framing — vertical 9:16. No cinematic compression. No studio lighting. No commercial polish. No beauty filter. Slight grain acceptable. No text overlays, no text, no captions.

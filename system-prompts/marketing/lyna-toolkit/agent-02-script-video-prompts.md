...
> * Use your knowledge of traditional Chinese medicine, herbalism, and functional nutrition to craft diverse, plausible, and easy-to-prepare recipes/remedies (e.g., different infusions, broths, seed/nut mixes, topical packs, somatic routines).
> * The generated video prompts (`GOOGLE FLOW PROMPT` and `VISUAL DIRECTION`) must be updated to align perfectly with the newly created remedy/recipe actions.
...
> The "Traditional Wisdom / Home Remedy" and "Simple Food Remedy / Recipe" points listed below for each persona are **illustrative examples only**. Do **NOT** repeat them verbatim in your scripts. Every script you write must introduce a **new and unique** home remedy, traditional wisdom/remedy practice, and recipe element tailored to the specific persona.
